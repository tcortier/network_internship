...
corr_LBM_main_VEM<-function(connectivity,completeness,proba_type,estimate_psi=FALSE,param,initMethod="hierarchical_clust",maxExplo=1.5,maxGroups=10,reinitialize=FALSE){
  ##Input parameters : 
  ...
  while(cond){
    if (gr==1){
      ...
      mods=mclapply(1:k[2],function(k2){##We evaluate all possible combinations with group k1
        ...
        model<-corrLBM_VEM(connectivity,completeness,k[1],k2,tau1,tau2,proba_type,estimate_psi,param)
        return(model)
      ...
      mods=mclapply(1:k[1],function(k1){
        ...
        tau1=clustering_indicator(cl0[[1]])
        tau2=clustering_indicator(cl0[[2]])
        model<-corrLBM_VEM(connectivity,completeness,k1,k[2],tau1,tau2,proba_type,estimate_psi,param)
        return(model)
      },mc.cores = param$cores)
    ...
    while(cond&(it<3)){##We repeat the operation a maximum of 3 times if we have improved the maximum ICL of all our models.
      ...
      for (k1 in 1:(k[1]-1)){
        for (k2 in 1:(k[2]-1)){
          ...
          model<-corrforward_explo(models,k1,k2,connectivity,completeness,proba_type,estimate_psi,param)##We explore all combinations and divide a group at random.
      ...
      for (k1 in c(k[1]:3)){
        for (k2 in c(k[2]:3)){
          ...
          model<-corrbackward_explo(models,k1,k2,connectivity,completeness,proba_type,estimate_psi,param)##All possibilities of grouping from k1 k2 onward are explored.
          if (model[[1]]$ICL>models[[paste(as.character(k1-1),as.character(k2),sep="&")]]$ICL){
